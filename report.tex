...
HDBSCAN, by contrast, is not gradient-based and has no differentiable loss. It is a combinatorial optimisation on the minimum spanning tree of mutual reachability distances from Eq.~\eqref{eq:mrd}. This is one reason HDBSCAN is theoretically preferable to $k$-means, whose Euclidean objective is non-convex and sensitive to initialisation.
...
\end{equation}
Eq.~\eqref{eq:mrd_props} is symmetric and non-negative; in practice it behaves as a semi-metric suitable for MST construction. Therefore Prim's or Kruskal's algorithm can be applied directly to obtain the cluster hierarchy graph used by HDBSCAN persistence selection.
